  ...
  <div className="grid grid-cols-2 gap-x-4">
    <div className="space-y-1">
      ...
      <DataRow
        icon={Timer}
        label="Takeoff"
        value={formatSeconds(flight.duration_takeoff_s)}
      />
      <DataRow
        icon={Timer}
        label="Climb"
        value={formatSeconds(flight.duration_climb_s)}
      />
      <DataRow
        icon={Timer}
        label="Cruise"
        value={formatSeconds(flight.duration_cruise_s)}
      />
      <DataRow
        icon={Timer}
        label="Descent"
        value={formatSeconds(flight.duration_descent_s)}
      />
      <DataRow
        icon={Timer}
        label="Landing"
        value={formatSeconds(flight.duration_landing_s)}
      />
    </div>
    <div className="space-y-1">
      ...
      <DataRow
        icon={Fuel}
        label="Takeoff"
        value={flight.fuel_takeoff_kg?.toLocaleString() ?? "N/A"}
      />
      <DataRow
        icon={Fuel}
        label="Climb"
        value={flight.fuel_climb_kg?.toLocaleString() ?? "N/A"}
      />
      <DataRow
        icon={Fuel}
        label="Cruise"
        value={flight.fuel_cruise_kg?.toLocaleString() ?? "N/A"}
      />
      <DataRow
        icon={Fuel}
        label="Descent"
        value={flight.fuel_descent_kg?.toLocaleString() ?? "N/A"}
      />
      <DataRow
        icon={Fuel}
        label="Landing"
        value={flight.fuel_landing_kg?.toLocaleString() ?? "N/A"}
      />
    </div>
      ...
      <div className="grid grid-cols-2 gap-x-4">
        <div>
          <DataRow
            icon={CloudCog}
            label="Takeoff"
            value={flight.co2_takeoff_kg?.toLocaleString() ?? "N/A"}
          />
          <DataRow
            icon={CloudCog}
            label="Climb"
            value={flight.co2_climb_kg?.toLocaleString() ?? "N/A"}
          />
          <DataRow
            icon={CloudCog}
            label="Cruise"
            value={flight.co2_cruise_kg?.toLocaleString() ?? "N/A"}
          />
        </div>
        <div>
          <DataRow
            icon={CloudCog}
            label="Descent"
            value={flight.co2_descent_kg?.toLocaleString() ?? "N/A"}
          />
          <DataRow
            icon={CloudCog}
            label="Landing"
            value={flight.co2_landing_kg?.toLocaleString() ?? "N/A"}
          />
          <DataRow
            icon={CloudCog}
            label="Per Passenger"
          ...
          <AccordionContent className="p-4 pt-0">
            <div className="space-y-1">
              <DataRow
                icon={Hash}
                label="Flight ID"
                value={flight.flight_id ?? "N/A"}
              />
              <DataRow
                icon={ShieldQuestion}
                label="FR24 ID"
                value={flight.fr24_id ?? "N/A"}
              />
              <DataRow
                icon={RadioTower}
                label="Callsign"
                value={flight.callsign ?? "N/A"}
              />
              <DataRow
                icon={Plane}
                label="Registration"
                value={flight.aircraft_reg ?? "N/A"}
              />
              <DataRow
                icon={Mountain}
                label="Distance"
                value={`${flight.distance_km?.toLocaleString() ?? "N/A"} km`}
              />
              <DataRow
                icon={Mountain}
              ...
              <DataRow
                icon={Timer}
                label="Created At"
                value={<DateTimeCell dateString={flight.created_at} />}
              />
